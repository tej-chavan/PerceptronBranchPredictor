	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  /*-----------------------------------------------------------------------------------------*/
  /* case statement for perceptron branch predictor.
  */
  /*-----------------------------------------------------------------------------------------*/
  ...
  case BPred2Level:
  case BPred2bit:
  case BPredPerceptron:                   // Adding case statement for branch predictor
    {
      int i;
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPred2Level:
	...
	fatal("level-2 size, `%d', must be non-zero and a power of two", 
      ...
      if (!shift_width || shift_width > 60)           // changing from 30 to 60 since we might have BHR length of 54
  ...
  /*-----------------------------------------------------------------------------------------------
   Code to check false inputs for perceptron predictor
  ------------------------------------------------------------------------------------------------*/  
  case BPredPerceptron:
    if (!l1size)
       fatal("number of perceptron, `%d', must be non-zero and non-negative", l1size);
    if (!l2size)
       fatal("number of perceptron, `%d', must be non-zero and non-negative", l2size);
    if (!shift_width)
       fatal("shift register width, `%d', must be non-zero and non-negative", shift_width);

    pred_dir->config.perceptron.wt_entries = l1size;
    pred_dir->config.perceptron.wt_bits = l2size;
  ...
  FILE *stream)			/* output stream */
{
  ...
  /*--------------------------------------------------------------------------
  ADDING PRINTS OFR PERCEPTRON
  ---------------------------------------------------------------------------*/  
  case BPredPerceptron:
  	fprintf(stream, "pred_dir: %s, %d perceptrons, %d Number of weight bits, %d GBHR length\n",
  	  name, pred_dir->config.perceptron.wt_entries, pred_dir->config.perceptron.wt_bits, pred_dir->config.perceptron.bhr_length);
  	break;
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  /*-------------------------------------------------------------------------------
  BTB prints for perceptron predictor
  --------------------------------------------------------------------------------*/  
  case BPredPerceptron:
		 ...
		 md_addr_t baddr)		/* branch address */
{
  ...
  switch (pred_dir->class) {
    ...
    /*--------------------------------------------------------------------------------------------
    	PREDICITION ALGORITHM FOR PERCEPTRON BRANCH PREDICTOR.
    	sum = 0;
					 ...
					 * used on mispredict recovery */
{
  ...
  switch (pred->class) {
    ...
    case BPred2bit:
    /*-------------------------------------------------------------------------
    ASSIGNING VALUES OF dir_update_ptr for perceptron
    --------------------------------------------------------------------------*/
    case BPredPerceptron:
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
