...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  *********************************** PERCEPTRON PREDICTOR CONFIGURATION ********************************/
  opt_reg_int_list(odb, "-bpred:perceptron",
                 "Neural Net Perceptron predictor config (<table size>)",
                 neunet_config, neunet_nelt, &neunet_nelt,
                 neunet_config, 
      ...
      int argc, char **argv)        /* command line arguments */
{
  ...
  /*----------------------------------------------------------------------------------------------------*/
  // PERCEPTRON FUNCTION CALL AND PASSING OF PARAMETERS.  
  /*----------------------------------------------------------------------------------------------------*/
  else if (!mystricmp(pred_type, "perceptron")) // give the same name 'perceptron' that you gave while declaring the configuration
    {
      /*Perceptron Branch predictor create. bpred_create() checks args*/
      pred = bpred_create(BPredPerceptron,
        /*bimod tabel size*/ 0,
        /* 2lev l1 size */neunet_config[0], // this is argument for number of entries in weight buffer 
        /* 2lev l2 size */neunet_config[1], // this is the argument for size of each weight buffer entry
        /* meta table size */0,
        /* history reg size */neunet_config[2], // this is the argument for size of GBHR, 
        /* history xor address */0,
        /* btb sets */btb_config[0],
        /* btb assoc */btb_config[1],
        /* ret-addr stack size */ras_size);
